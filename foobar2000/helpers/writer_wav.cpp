...
#ifdef _WIN32 // FIX ME this requires winmm types
#if 0
#include <ks.h>
#include <ksmedia.h>
#endif
...
void CWavWriter::writeID(const GUID & id, abort_callback & abort) {
	if (is64()) {
		...
		for(t_size walk = 0; walk < _countof(RIFF_chunks); ++walk) {
...
static t_uint32 calculate_chunk_fix(t_uint64 p_value)
{
	return (t_uint32) p_value;//deprecated
}
